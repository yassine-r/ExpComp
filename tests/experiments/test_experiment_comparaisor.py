...
def test_custom_condition_with_function(comparison):
    ...
    custom_condition = Condition(
        key="custom_filter", operator="custom", value=None, custom_fn=custom_filter
    )
...
def test_experiment_with_missing_metric(mock_experiments):
    ...
    config = ExperimentConfig(
        id="exp_missing", model_type="model_x", learning_rate=0.05
    )
    metrics = [
    ...
    assert len(df) == 4
    assert pd.isna(
        df.loc[df["experiment_id"] == "exp_missing", "metric_f1_score"]
    ).all()
    assert pd.isna(
        df.loc[df["experiment_id"] == "exp_missing", "metric_precision"]
    ).all()
