...
class Metric:
    ...
    def __init__(self, experiment_id: Any, name: str, value: Optional[Union[float, int, List]] = None, **kwargs):
        """
...
if __name__ == "__main__":
    # Create a metric for an experiment
    ...
    # Access attributes
    print(f"Metric: {accuracy_metric.name}")
    print(f"Value: {accuracy_metric.value}")
    print(f"Associated with experiment: {accuracy_metric.experiment_id}")
    
    # Update the metric value
    accuracy_metric.update_value(0.94)
    print(f"Updated value: {accuracy_metric.value}")
    
    # Update additional attributes
    accuracy_metric.update(timestamp="2025-02-26", notes="After hyperparameter tuning")
    
    # Export to JSON
    metric_json = accuracy_metric.to_json()
    print(f"\nMetric JSON:\n{metric_json}")
    
    # Create a new metric from a dictionary
    loss_metric = Metric.from_dict({
        "experiment_id": "56788",
        "name": "loss",
        "value": 0.12,
        "direction": "minimize"
